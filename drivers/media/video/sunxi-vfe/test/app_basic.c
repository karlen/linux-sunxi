...
//	printf ("buf.m.offset = 0x%x\n",buf.m.offset);		
	...
	if(count == 60)
    {
        ...
        if(buf_size_1 != 0)
        {
		    ...
		    fwrite(buffers[buf.index].start, buf_size_1, 1, file_fd); //½«Ǥдɫτ¼�   fclose(file_fd);
		}
        if(buf_size_2 != 0)
        {
		    file_fd = fopen("/mnt/fb2.bin","w");
		    fwrite(buffers[buf.index].start+ALIGN_4K(buf_size_1), buf_size_2, 1, file_fd); //½«Ǥдɫτ¼�   fclose(file_fd);
	    }
        if(buf_size_3 != 0)
        {
		    file_fd = fopen("/mnt/fb3.bin","w");
		    fwrite(buffers[buf.index].start+ALIGN_4K(buf_size_1+buf_size_2), buf_size_3, 1, file_fd); //½«Ǥдɫτ¼�}
	    }
	}
