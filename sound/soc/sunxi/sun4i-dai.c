...
static void sun4i_dai_start_capture(struct sun4i_dai *sdai)
{
	/* Flush RX FIFO */
        regmap_update_bits(sdai->regmap, SUN4I_DAI_FIFO_CTRL_REG,
			   SUN4I_DAI_FIFO_CTRL_FLUSH_RX,
			   SUN4I_DAI_FIFO_CTRL_FLUSH_RX);

        /* Clear RX counter */
	regmap_write(sdai->regmap, SUN4I_DAI_RX_CNT_REG, 0);

        /* Enable RX Block */
        regmap_update_bits(sdai->regmap, SUN4I_DAI_CTRL_REG,
			   SUN4I_DAI_CTRL_RX_EN,
			   SUN4I_DAI_CTRL_RX_EN);

        /* Enable RX DRQ */
        regmap_update_bits(sdai->regmap, SUN4I_DAI_DMA_INT_CTRL_REG,
			   SUN4I_DAI_DMA_INT_CTRL_RX_DRQ_EN,
			   SUN4I_DAI_DMA_INT_CTRL_RX_DRQ_EN);
}
...
static void sun4i_dai_stop_capture(struct sun4i_dai *sdai)
{
        /* Disable RX Block */
        regmap_update_bits(sdai->regmap, SUN4I_DAI_CTRL_REG,
			   SUN4I_DAI_CTRL_RX_EN,
...
static int sun4i_dai_probe(struct platform_device *pdev)
{
	...
	sdai->playback_dma_data.addr = res->start + SUN4I_DAI_FIFO_RX_REG;
	sdai->playback_dma_data.maxburst = 4;
